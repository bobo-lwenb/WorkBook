...
class NotificationDemoActivity : AppCompatActivity(), View.OnClickListener {
    ...
    private var basic: TextView? = null
        ...
        when (v?.id) {
            R.id.notifi_basic -> {
                ...
                val builder = NotificationCompat.Builder(this, CHANEEL_ID)
                        ...
                        .setStyle(NotificationCompat.BigTextStyle().bigText("Notice that the NotificationCompat.Builder constructor requires that you provide a channel ID. This is required for compatibility with Android 8.0 (API level 26) and higher, but is ignored by older versions."))
            ...
            R.id.notifi_reply -> {
                ...
                val replyPendingIntent: PendingIntent = PendingIntent.getBroadcast(applicationContext, 0, receiverIntent, PendingIntent.FLAG_UPDATE_CURRENT)

                val action: NotificationCompat.Action = NotificationCompat.Action.Builder(R.drawable.ic_icon,
                        "reply1",// 回复按钮文本
                        replyPendingIntent)
